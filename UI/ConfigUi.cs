...
public sealed class ConfigUi
{
    ...
    private readonly Services.AllaganToolsOwnershipService allaganTools;
    private bool isOpen;
    private string outputDirectory;

    public ConfigUi(Configuration configuration, Services.AllaganToolsOwnershipService allaganTools)
    {
    ...
    public void Draw()
    {
        ...
        if (ImGui.BeginTabBar("GlamSpectorSettingsTabs"))
        {
            if (ImGui.BeginTabItem("General"))
            {

                ImGui.TextWrapped("GlamSpector captures inspected glamours into a preview-first Library. M3.16 can optionally supplement positive ownership evidence through Allagan Tools' local Dalamud IPC, while unverified results remain unknown and all Library/Wanted metadata stays local.");
                ImGui.Spacing();
    ...
    {
        ImGui.TextWrapped("External-plugin integrations are optional, independently configured, and disabled by default. GlamSpector's core capture and Library features never require them.");
        ImGui.Spacing();

        if (!ImGui.BeginTabBar("GlamSpectorIntegrationPlugins"))
            return;

        if (ImGui.BeginTabItem("Allagan Tools"))
        {
            ImGui.TextWrapped("Allagan Tools can provide additional positive ownership verification from locally cached personal inventory and storage.");
            ImGui.Spacing();

            var enabled = configuration.EnableAllaganToolsIntegration;
            if (ImGui.Checkbox("Enable Allagan Tools integration", ref enabled))
                configuration.EnableAllaganToolsIntegration = enabled;

            ImGui.TextUnformatted($"Status: {allaganTools.StatusText}");
            var scan = allaganTools.GetScanProgress();
            if (enabled && scan.HasSnapshot)
            {
                if (scan.Active)
                {
                    ImGui.TextUnformatted($"Ownership scan: {scan.Completed} / {scan.Total}");
                    var fraction = scan.Total == 0 ? 1f : System.Math.Clamp((float)scan.Completed / scan.Total, 0f, 1f);
                    ImGui.ProgressBar(fraction, new System.Numerics.Vector2(-1f, 0f), $"{scan.Completed} / {scan.Total}");
                }
                else
                {
                    ImGui.TextDisabled("Allagan Tools personal cache ready");
                }
            }
            ImGui.Spacing();
            ImGui.TextWrapped("Uses local Dalamud IPC only. No data is uploaded or shared. A zero ownership result remains unverified (?). Allagan Tools is not required for GlamSpector.");
